...
export default class SourcefetchView {
  ...
  constructor(serializedState) {
    ...
    heading.textContent = "CODE SPAN WORST GUI"
    ...
    // let length of response
    // const response = ['import math' , '\nfrom bs4 import BeautifulSoup' , '\nimport requests'];
    // for (var i = 0; i < response.length; i++) {
    //   const codeBlock = document.createElement('code');
    //   codeBlock.classList.add('codeBlock');
    //   codeBlock.textContent = response[i];
    //   preBlock.appendChild(codeBlock);
    // }
    // repeatableUnits.appendChild(preBlock);
    // repeatableUnits.appendChild(source);
    ...
    for ( let parentCodeIndex=0 ; parentCodeIndex < JSONdata[0].length ; parentCodeIndex++){
      ...
      switch(JSONdata[0][parentCodeIndex][0]) {
        ...
        case "two" :
          const codeBlock = document.createElement('code')
          codeBlock.classList.add('codeBlock')
          codeBlock.textContent = JSONdata[0][parentCodeIndex][1]
          preBlock.appendChild(codeBlock)
      ...
      this.attachEventListenerToLeftWrapper(upVoteBlock,downVoteBlock,bugBlock,copyBlock,pasteBlock,preBlock);
  ...
  attachEventListenerToLeftWrapper(up, down, bug, copy, paste, preBlock) {
